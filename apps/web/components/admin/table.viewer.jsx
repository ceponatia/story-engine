...
import { getTableData, deleteTableRows } from "@/lib/actions/admin";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow, } from "@/components/ui/table";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Checkbox } from "@/components/ui/checkbox";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger, } from "@/components/ui/alert-dialog";
export function TableViewer({ tableName, data, onRefresh }) {
    ...
    const toggleSelectAll = () => {
        if (selectedRows.size === tableData.rows.length) {
            ...
            const allKeys = new Set(tableData.rows
                .map((row) => JSON.stringify(getPrimaryKeyValue(row)))
                .filter((key) => key !== "null"));
    ...
    return (<div className="space-y-6">
      ...
      <div className="border rounded-lg">
        ...
        <div className="max-h-64 overflow-y-auto p-4 bg-card rounded-b-lg">
          <div className="space-y-2">
            {tableData.columns.map((column) => (<div key={column.column_name} className="flex items-center justify-between p-2 border rounded">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1">
                    {column.is_primary_key && (<div title="Primary Key">
                        <Key className="h-3 w-3 text-yellow-600"/>
                      </div>)}
                    {column.is_foreign_key && (<div title="Foreign Key">
                        <Link className="h-3 w-3 text-blue-600"/>
                      </div>)}
                    <span className="font-medium">{column.column_name}</span>
                  </div>
                  <Badge variant="secondary" className={`text-xs ${getDataTypeColor(column.data_type)}`}>
                    {column.data_type}
                  </Badge>
                  {column.is_nullable === "YES" && (<Badge variant="outline" className="text-xs">
                      nullable
                    </Badge>)}
                </div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  {column.column_default && <span>default: {column.column_default}</span>}
                  {column.foreign_table && (<span>
      ...
      <div className="border rounded-lg">
        <div className="flex items-center justify-between p-4 border-b bg-card rounded-t-lg">
          ...
          <div className="flex items-center gap-2">
            {selectedRows.size > 0 && (<AlertDialog>
            ...
            {totalPages > 1 && (<div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => loadPage(currentPage - 1)} disabled={currentPage === 1 || loading}>
                  Previous
                </Button>
                <span className="text-sm text-muted-foreground">
                  Page {currentPage} of {totalPages}
                </span>
                <Button variant="outline" size="sm" onClick={() => loadPage(currentPage + 1)} disabled={currentPage === totalPages || loading}>
                  Next
                </Button>
              </div>)}
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/30 text-sm text-muted-foreground">
          <span>
            ...
            {totalPages > 1 &&
            ` (${(currentPage - 1) * rowsPerPage + 1}-${Math.min(currentPage * rowsPerPage, tableData.total_rows)} of ${tableData.total_rows})`}
          </span>
        </div>
        <div className="max-h-96 overflow-auto bg-card rounded-b-lg">
          {tableData.rows.length === 0 ? (<div className="flex items-center justify-center h-32 text-muted-foreground">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="sticky left-0 z-10 bg-background border-r w-[50px]">
                      <Checkbox checked={selectedRows.size === tableData.rows.length && tableData.rows.length > 0} onCheckedChange={toggleSelectAll} disabled={tableData.rows.length === 0}/>
                    </TableHead>
                    {tableData.columns.map((column) => (<TableHead key={column.column_name} className="min-w-[120px] whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          {column.is_primary_key && (<div title="Primary Key">
                              <Key className="h-3 w-3 text-yellow-600"/>
                            </div>)}
                    ...
                    <TableHead className="sticky right-0 z-10 bg-background border-l w-[50px]">
                      Actions
                    </TableHead>
                  </TableRow>
                </TableHeader>
                ...
                return (<TableRow key={index} className={isSelected ? "bg-muted/50" : ""}>
                        <TableCell className={`sticky left-0 z-10 border-r ${isSelected ? "bg-muted/50" : "bg-background"}`}>
                          <Checkbox checked={isSelected} onCheckedChange={() => toggleRowSelection(row)} disabled={getPrimaryKeyValue(row) === null}/>
                        </TableCell>
                        {tableData.columns.map((column) => (<TableCell key={column.column_name} className="min-w-[120px] max-w-[200px]">
                        ...
                        <TableCell className={`sticky right-0 z-10 border-l ${isSelected ? "bg-muted/50" : "bg-background"}`}>
                          <Button variant="ghost" size="sm" onClick={() => copyToClipboard(JSON.stringify(row, null, 2))} title="Copy row as JSON">
