...
import { TableData, getTableData, deleteTableRows } from "@/lib/actions/admin";
import {
  Table,
  TableBody,
  TableCell,
...
export function TableViewer({ tableName, data, onRefresh }: TableViewerProps) {
  ...
  const getPrimaryKeyValue = (row: Record<string, unknown>): unknown => {
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="sticky left-0 z-10 bg-background border-r w-[50px]">
                      <Checkbox
                        checked={
                          selectedRows.size === tableData.rows.length && tableData.rows.length > 0
                        }
                  ...
                  {tableData.rows.map((row, index) => {
                    ...
                    return (
                      <TableRow key={index} className={isSelected ? "bg-muted/50" : ""}>
                        <TableCell
                          className={`sticky left-0 z-10 border-r ${isSelected ? "bg-muted/50" : "bg-background"}`}
                        >
                          <Checkbox
                            checked={isSelected}
                        ...
                        {tableData.columns.map((column) => (
                          ...
                          >
                            <div
                              className="truncate"
                              title={formatCellValue(row[column.column_name])}
                            >
                        ...
                        <TableCell
                          className={`sticky right-0 z-10 border-l ${isSelected ? "bg-muted/50" : "bg-background"}`}
                        >
                          <Button
                            variant="ghost"
                            size="sm"
