...
export function AdventureTypeForm({ adventureType, isEditing = false }) {
    const router = useRouter();
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [formData, setFormData] = useState({
        name: (adventureType === null || adventureType === void 0 ? void 0 : adventureType.name) || "",
        description: (adventureType === null || adventureType === void 0 ? void 0 : adventureType.description) || "",
        is_public: (adventureType === null || adventureType === void 0 ? void 0 : adventureType.is_public) || false,
        template_content: (adventureType === null || adventureType === void 0 ? void 0 : adventureType.template_content) || DEFAULT_TEMPLATE,
    });
    ...
    const handleTemplateChange = (value) => {
        setFormData((prev) => (Object.assign(Object.assign({}, prev), { template_content: value })));
    };
    const insertPlaceholder = (placeholder) => {
        ...
        if (textarea) {
            ...
            setFormData((prev) => (Object.assign(Object.assign({}, prev), { template_content: newText })));
            setTimeout(() => {
                textarea.focus();
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input id="name" value={formData.name} onChange={(e) => setFormData((prev) => (Object.assign(Object.assign({}, prev), { name: e.target.value })))} placeholder="e.g., fantasy, mystery, horror" required disabled={loading}/>
          ...
          <div>
            ...
            <Textarea id="description" value={formData.description} onChange={(e) => setFormData((prev) => (Object.assign(Object.assign({}, prev), { description: e.target.value })))} placeholder="Describe what makes this adventure type unique..." rows={3} disabled={loading}/>
          </div>

          <div className="flex items-center space-x-2">
            <Checkbox id="is_public" checked={formData.is_public} onCheckedChange={(checked) => setFormData((prev) => (Object.assign(Object.assign({}, prev), { is_public: !!checked })))} disabled={loading}/>
          ...
          <Tabs defaultValue="editor" className="w-full">
            ...
            <TabsContent value="editor" className="space-y-4">
              <div>
                ...
                <Textarea id="template_content" value={formData.template_content} onChange={(e) => handleTemplateChange(e.target.value)} placeholder="Enter your template content..." rows={20} className="font-mono text-sm" required disabled={loading}/>
                <p className="text-sm text-muted-foreground mt-1">
            ...
            <TabsContent value="help" className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {placeholders.map((placeholder) => (<Button key={placeholder} type="button" variant="outline" size="sm" onClick={() => insertPlaceholder(placeholder)} className="justify-start font-mono text-xs">
                      {placeholder}
                    </Button>))}
                </div>
              </div>
