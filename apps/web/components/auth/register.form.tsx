...
import { registerFormSchema, type RegisterFormInput } from "@story-engine/validation";
...
export function RegisterForm() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      const errorMessage =
        err?.message || err?.details?.message || "Something went wrong. Please try again.";
      setError(errorMessage);
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {error && (
              <div className="text-sm text-red-600 text-center bg-red-50 p-3 rounded">{error}</div>
            )}
