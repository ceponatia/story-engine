...
export function LibraryBreadcrumbs({ type, itemName, itemId }) {
    ...
    return (<Breadcrumb className="mb-4">
      <BreadcrumbList>
        ...
        {type && (<>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              {isNewPage || isEditPage ? (<BreadcrumbLink asChild>
                  <Link href={`/library/${type}`}>{typeDisplayNames[type] || type}</Link>
                </BreadcrumbLink>) : (<BreadcrumbPage>{typeDisplayNames[type] || type}</BreadcrumbPage>)}
            </BreadcrumbItem>
          </>)}
