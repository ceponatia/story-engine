...
export function UnifiedLocationManager({ 
  ...
  const canEdit = !location || currentUser?.id === location.created_by;
  ...
  const getFormDataFromLocation = useCallback((loc: Location): LocationFormData => ({
    name: loc.name,
    description: loc.description,
    notable_features: Array.isArray(loc.notable_features) 
      ? loc.notable_features.join(", ") 
      : "",
    connected_locations: Array.isArray(loc.connected_locations) 
      ? loc.connected_locations.join(", ") 
      : "",
    tags: Array.isArray(loc.tags) 
      ? loc.tags.join(", ") 
      : "",
  }), []);
  ...
  // Mode change handler
  const handleModeChange = useCallback((newMode: 'create' | 'view' | 'edit') => {
    setMode(newMode);
    setError(null);
    ...
    try {
      const transformedData = {
        ...
        notable_features: data.notable_features 
          ? data.notable_features.split(',').map(s => s.trim()).filter(Boolean) 
          : undefined,
      ...
      if (mode === 'create') {
        const newLocation = await createLocationAction(transformedData);
        onLocationUpdate?.(newLocation);
        handleModeChange('view');
      } else if (mode === 'edit' && location) {
        const updatedLocation = await updateLocationAction(location.id, transformedData);
