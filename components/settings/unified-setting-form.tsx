...
import { Setting } from "@/lib/database/types";
...
const WORLD_TYPE_OPTIONS = ["Fantasy", "Sci-Fi", "Modern", "Historical", "Post-Apocalyptic", "Cyberpunk", "Steampunk", "Other"];
...
export function UnifiedSettingForm({ 
  ...
  const handleCancel = () => {
    if (mode === 'create') {
      ...
      // Reset form data to original setting data
      if (setting) {
        setFormData({
          name: setting.name ?? "",
          description: setting.description ?? "",
