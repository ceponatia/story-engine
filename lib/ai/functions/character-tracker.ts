...
function generateStateEvents(updates: Record<string, StateUpdate>): CharacterStateEvent[] {
  ...
  Object.values(updates).forEach(update => {
    const type = update.field.includes('appearance') ? 'appearance' :
                 update.field.includes('personality') ? 'personality' :
                 update.field.includes('scents') ? 'scents' : 'other';
  ...
  Object.entries(updatesByType).forEach(([type, typeUpdates]) => {
    ...
    if (highConfidenceUpdates.length > 0) {
      const descriptions = highConfidenceUpdates.map(u => 
        u.context.split(' - ')[1] || 'Character change detected'
      );
...
export async function updateCharacterFromExtractions(
  ...
  try {
    ...
    const existingState = await getAdventureCharacterState(adventureId, userId);
    ...
    result.highConfidenceUpdates = Object.values(mergedUpdates)
      .filter(u => u.confidence === 'high').length;
    ...
    if (!dryRun && Object.keys(mergedUpdates).length > 0) {
      ...
      const finalState = {
        ...existingState,
        ...Object.fromEntries(
          Object.entries(mergedUpdates).map(([key, update]) => [key, update])
        )
      };
      
      // Update character state in database
      await updateAdventureCharacterState(adventureId, finalState, userId);
...
export async function getCharacterStateHistory(
  ...
  try {
    const stateData = await getAdventureCharacterState(adventureId, userId);
...
export function analyzeCharacterDevelopment(history: StateUpdate[]): {
  ...
  analysis.recentActivity = history.some(
    update => new Date(update.timestamp) > oneDayAgo
  );
  
  // Generate trend insights
  const mostActiveType = Object.entries(analysis.changesByType)
    .sort(([,a], [,b]) => b - a)[0];
