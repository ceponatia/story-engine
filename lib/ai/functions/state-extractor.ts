...
function extractAdvancedPatterns(text: string): StateExtraction[] {
  ...
  const contextualClues = [
    { pattern: /(?:location|place|room|area).*?(?:changed?|moved?|went?|entered?)/gi, type: 'location' as const },
    { pattern: /(?:personality|character|nature).*?(?:changed?|shifted?|became?)/gi, type: 'personality' as const },
    { pattern: /(?:appearance|look|style).*?(?:changed?|different|new)/gi, type: 'appearance' as const }
  ];
...
export async function extractStateFromResponse(
  ...
  const uniqueExtractions = filteredExtractions.filter((ext, index, arr) => 
    arr.findIndex(other => 
      JSON.stringify(other.extractedData) === JSON.stringify(ext.extractedData)
    ) === index
  );
 ...
 */
export function getExtractionConfig(mode: 'conservative' | 'balanced' | 'aggressive'): Partial<StateExtractionConfig['enabledFeatures']> {
  switch (mode) {
    case 'conservative':
      return {
        simplePatterns: true,
