...
export type AttributeType = 'appearance' | 'personality' | 'scents_aromas' | 'location_features' | 'setting_elements';

/**
 * Main unified parser function that handles all attribute types
 */
export function parseAttributeText(text: string, attributeType: AttributeType): UnifiedParserResult {
  if (!text || text.trim() === '') {
    return {};
  }
  ...
  for (let i = 0; i < keyPositions.length; i++) {
    ...
    const valueText = text.slice(valueStart, valueEnd).replace(/^\s*,?\s*/, '').replace(/\s*,?\s*$/, '').trim();
    
    if (valueText) {
      matches.push([`${currentKey.key}: ${valueText}`, currentKey.key, valueText] as RegExpMatchArray);
    }
  }
...
function parseValues(valueString: string): string[] {
  return valueString
    ...
    .map(value => {
      ...
      return value
        .replace(/\b(scent|smell|aroma|fragrance|odor|perfume)\b/gi, '')
        .trim()
    })
    .filter(value => value.length > 0)
 ...
 */
function createNamespacedKey(category: string, values: string[], attributeType: AttributeType): string {
...
function inferAppearanceSubType(valueString: string): string {
  ...
  const colorWords = ['red', 'blue', 'green', 'yellow', 'black', 'white', 'brown', 'blonde', 'brunette', 'gray', 'grey', 'pink', 'purple', 'orange', 'silver', 'golden', 'dark', 'light', 'pale', 'tan', 'olive', 'fair'];
  if (colorWords.some(color => valueString.includes(color))) {
    return 'color';
  }
  
  // Length-related descriptors (especially for hair)
  const lengthWords = ['long', 'short', 'medium', 'shoulder', 'waist', 'ankle', 'floor', 'chin', 'ear', 'neck', 'length'];
  if (lengthWords.some(length => valueString.includes(length))) {
    return 'length';
  }
  
  // Style-related descriptors (especially for hair)
  const styleWords = ['ponytail', 'bob', 'pixie', 'bangs', 'layers', 'braids', 'curls', 'waves', 'straight', 'updo', 'bun', 'pigtails', 'buzz', 'crew', 'fade'];
  if (styleWords.some(style => valueString.includes(style))) {
    return 'style';
  }
  
  // Texture-related descriptors
  const textureWords = ['smooth', 'rough', 'soft', 'hard', 'silky', 'coarse', 'fine', 'thick', 'thin', 'bumpy', 'wrinkled', 'glossy', 'matte', 'shiny', 'dull', 'curly', 'wavy'];
  if (textureWords.some(texture => valueString.includes(texture))) {
    return 'texture';
  }
  
  // Size-related descriptors
  const sizeWords = ['big', 'small', 'large', 'tiny', 'huge', 'massive', 'petite', 'giant', 'mini', 'enormous', 'tall', 'wide', 'narrow', 'broad', 'slim'];
  if (sizeWords.some(size => valueString.includes(size))) {
    return 'size';
  }
  
  // Shape-related descriptors
  const shapeWords = ['round', 'square', 'oval', 'circular', 'angular', 'curved', 'pointed', 'blunt', 'sharp', 'flat', 'rounded'];
...
function inferPersonalitySubType(valueString: string): string {
  ...
  const emotionWords = ['happy', 'sad', 'angry', 'excited', 'nervous', 'anxious', 'calm', 'peaceful', 'joyful', 'melancholy', 'furious', 'content', 'worried', 'relaxed', 'stressed'];
  if (emotionWords.some(emotion => valueString.includes(emotion))) {
    return 'emotions';
  }
  
  // Quirks/habits descriptors
  const quirkWords = ['twitches', 'hums', 'talks', 'collects', 'always', 'never', 'habit', 'tends', 'obsessed', 'compulsive', 'repetitive'];
  if (quirkWords.some(quirk => valueString.includes(quirk))) {
    return 'quirks';
  }
  
  // Fear descriptors
  const fearWords = ['afraid', 'scared', 'terrified', 'phobic', 'fearful', 'panicked', 'frightened', 'timid', 'cowardly', 'dreads'];
  if (fearWords.some(fear => valueString.includes(fear))) {
    return 'fears';
  }
  
  // Desire descriptors
  const desireWords = ['wants', 'wishes', 'dreams', 'hopes', 'longs', 'craves', 'yearns', 'desires', 'seeks', 'pursues', 'ambitious'];
  if (desireWords.some(desire => valueString.includes(desire))) {
    return 'desires';
  }
  
  // Motivation descriptors
  const motivationWords = ['driven', 'motivated', 'determined', 'goal-oriented', 'focused', 'dedicated', 'committed', 'passionate', 'inspired'];
...
function inferScentSubType(valueString: string): string {
  ...
  const fragranceWords = ['floral', 'sweet', 'vanilla', 'rose', 'jasmine', 'lavender', 'citrus', 'fresh', 'clean', 'perfumed', 'pleasant', 'lovely', 'delicate', 'subtle'];
  if (fragranceWords.some(fragrance => valueString.includes(fragrance))) {
    return 'fragrance';
  }
  
  // Strong/distinctive scent descriptors
  const aromaWords = ['spicy', 'warm', 'rich', 'deep', 'intense', 'strong', 'distinctive', 'exotic', 'earthy', 'woody', 'herbal'];
  if (aromaWords.some(aroma => valueString.includes(aroma))) {
    return 'aroma';
  }
  
  // Unpleasant/body odor descriptors
  const odorWords = ['sweaty', 'musky', 'sour', 'tangy', 'cheesy', 'stinky', 'funky', 'ripe', 'pungent', 'acrid', 'bitter', 'sharp', 'smelly'];
...
function inferLocationSubType(valueString: string): string {
  ...
  const structureWords = ['building', 'structure', 'tower', 'wall', 'bridge', 'gate', 'door', 'window', 'roof', 'floor', 'stairs', 'pillar', 'column', 'arch'];
  if (structureWords.some(structure => valueString.includes(structure))) {
    return 'structures';
  }
  
  // Natural features
  const naturalWords = ['river', 'mountain', 'hill', 'forest', 'tree', 'lake', 'ocean', 'valley', 'cave', 'rock', 'stone', 'grass', 'flower', 'plant', 'garden'];
  if (naturalWords.some(natural => valueString.includes(natural))) {
    return 'natural';
  }
  
  // Atmosphere and ambiance
  const atmosphereWords = ['dark', 'bright', 'quiet', 'loud', 'peaceful', 'chaotic', 'mysterious', 'welcoming', 'threatening', 'ancient', 'modern', 'magical'];
  if (atmosphereWords.some(atmosphere => valueString.includes(atmosphere))) {
    return 'atmosphere';
  }
  
  // Resources and materials
  const resourceWords = ['gold', 'silver', 'metal', 'wood', 'stone', 'crystal', 'gem', 'water', 'food', 'treasure', 'artifact', 'weapon', 'tool'];
...
function inferSettingSubType(valueString: string): string {
  ...
  const genreWords = ['fantasy', 'medieval', 'modern', 'futuristic', 'sci-fi', 'cyberpunk', 'steampunk', 'victorian', 'ancient', 'prehistoric', 'magical', 'technological'];
  if (genreWords.some(genre => valueString.includes(genre))) {
    return 'genre';
  }
  
  // Political and social systems
  const politicalWords = ['kingdom', 'empire', 'republic', 'democracy', 'monarchy', 'dictatorship', 'guild', 'clan', 'tribe', 'government', 'ruler', 'law'];
  if (politicalWords.some(political => valueString.includes(political))) {
    return 'political';
  }
  
  // Cultural elements
  const culturalWords = ['tradition', 'custom', 'ritual', 'ceremony', 'festival', 'religion', 'belief', 'language', 'art', 'music', 'dance', 'cuisine'];
  if (culturalWords.some(cultural => valueString.includes(cultural))) {
    return 'cultural';
  }
  
  // Economic systems
  const economicWords = ['trade', 'merchant', 'market', 'currency', 'gold', 'silver', 'barter', 'wealth', 'poor', 'rich', 'resource', 'mining', 'farming'];
  if (economicWords.some(economic => valueString.includes(economic))) {
    return 'economic';
  }
  
  // Historical events and lore
  const historyWords = ['war', 'battle', 'conflict', 'peace', 'alliance', 'treaty', 'legend', 'myth', 'prophecy', 'ancient', 'past', 'history'];
  ...
  switch (attributeType) {
    case 'appearance':
      return {
        ...
        hair_style: ['hair style', 'hairstyle', 'hair cut', 'haircut'],
        ...
        breast_size: ['breast size', 'chest size', 'bust size'],
        feet_size: ['feet size', 'foot size']
      };
...
function extractDescriptors(sentence: string, attributeType: AttributeType): string[] {
  ...
  const skipWords = [
    'the', 'and', 'but', 'with', 'has', 'have', 'had', 'was', 'were', 'is', 'are',
    'very', 'quite', 'really', 'rather', 'somewhat', 'appears', 'seems', 'looks'
  ];

  return words
    .filter(word => !skipWords.includes(word))
    .map(word => formatTagName(word));
}
...
export function attributeToText(attributeData: UnifiedParserResult): string {
  ...
  Object.entries(attributeData).forEach(([namespacedKey, values]) => {
    ...
    const formattedSubType = subType === 'appearance' || subType === 'traits' || subType === 'scents' ? '' : ` (${subType})`;
 ...
 */
export function mergeAttributes(existing: UnifiedParserResult, additional: UnifiedParserResult): UnifiedParserResult {
