...
export function getModelRecommendation(availableVRAM) {
    if (availableVRAM >= 8) {
...
export function validateCudaSupport() {
    return Boolean(process.env.CUDA_VISIBLE_DEVICES !== "-1" &&
        typeof process.env.CUDA_VISIBLE_DEVICES !== "undefined");
}
export function validateRocmSupport() {
    return Boolean(process.env.ROCR_VISIBLE_DEVICES !== "-1" &&
