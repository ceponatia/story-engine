...
import type { ProviderAdapter, ChatMessage, CompletionDelta, CompletionFinal, CompletionOptions } from '../src/types';

class MockAdapter implements ProviderAdapter {
  async capabilities(model: string) { return { id: `mock:${model}` }; }
  async chat(messages: ChatMessage[], opts: CompletionOptions) {
    async function* stream(): AsyncIterable<CompletionDelta> {
      yield { type: 'text', text: 'Hello' };
      yield { type: 'text', text: ', world' };
    }
    const final: Promise<CompletionFinal> = Promise.resolve({ model: opts.model, finishReason: 'stop', usage: {} });
...
describe('LLMClient.completeChat', () => {
  it('streams deltas and resolves final', async () => {
    ...
    const res = await client.completeChat({ messages: [{ role: 'user', content: 'Hi' }], options: { model: 'test' } });
    let text = '';
