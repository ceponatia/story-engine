 ...
 *
 * Base interfaces for repository pattern implementation with multi-database support.
 * Provides consistent patterns for CRUD operations, caching, health monitoring,
 * and transaction coordination across all entity repositories.
 */

import { IMultiDatabaseManager, Result } from "./databaseManager.interface";
...
export interface IAdvancedRepository<T, ID>
  extends IRepositoryWithCache<T, ID>,
    IRepositoryWithTransaction<T, ID> {
  ...
  updateMany(updates: Array<{ id: ID; data: UpdateDTO<T> }>): Promise<T[]>;
...
// Specialized Repository Interfaces for Specific Entities
