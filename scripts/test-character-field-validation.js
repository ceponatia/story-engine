  ...
  try {
    ...
    if (rulesResult.rows.length > 0) {
      ...
      rulesResult.rows.forEach(rule => {
        console.log(`   ${rule.field_path} (${rule.protection_level}, min confidence: ${rule.min_confidence})`);
      });
    } else {
    ...
    if (!lowConfidenceResult.allowed && lowConfidenceResult.reason.includes('Confidence score too low')) {
